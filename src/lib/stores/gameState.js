...
function initSocket() {
  import('socket.io-client').then(({ io }) => {
    ...
    socket.on('connect_error', () => {
      lobbyError.set("Impossible de se connecter au serveur")
    })
...
export function createRoom(className) {
  if (checkSocket()) socket.emit('create', className)
}

export function joinRoom(code, className) {
  if (checkSocket()) socket.emit('join', { code: code.trim().toUpperCase(), className })
}

export function startGame() {
  if (checkSocket()) socket.emit('start')
}

export function dispatch(action) {
  if (checkSocket()) socket.emit('action', action)
}
